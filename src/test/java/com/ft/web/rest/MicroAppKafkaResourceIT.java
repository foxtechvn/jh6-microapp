...
import com.ft.config.KafkaProperties;
...
class MicroAppKafkaResourceIT {
    ...
    void setup() {
        ...
        MicroAppKafkaResource kafkaResource = new MicroAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/micro-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/micro-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
